...
function determineShards(decade, topic) {
  ...
  if (decade) {
    ...
    ["1960s", "1970s", "1980s", "1990s", "2000s", "2010s", "2020s"].forEach(
      (d) => {
...
function executeSearch(
  ...
  shardCache.forEach((shard, path) => {
    ...
    shard.sentences.forEach((sentence) => {
      ...
      if (
        decadeFilter &&
        !sentence.year.toString().startsWith(decadeFilter.slice(0, 3))
      ) {
        return;
      }
...
function sortResults(results, sortBy) {
  switch (sortBy) {
    ...
    case "minister":
      results.sort((a, b) => {
        const ministerCompare = (a.minister || "").localeCompare(
          b.minister || "",
        );
            ...
            <div class="search-result-meta">
                <span class="result-info">
                    <strong>${result.year}</strong> · ${
                      result.minister || "Unknown Minister"
                    }
                </span>
...
async function toggleContext(index) {
  ...
  const button = document.querySelector(
    `[data-index="${index}"] .context-toggle`,
  );
